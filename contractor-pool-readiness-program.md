# Contractor Pool Onboarding & Readiness
...
It is written to be lifted and re-run: the architecture, module map, assessment gates,
operating cadence, and asset inventory are all portable to any organization that staffs
learning work through a contingent talent pool.
...
| **Completion gate** | Two knowledge checks + two acknowledgments + a current availability record |
| **Outcome** | A staffing-ready bench that can be matched to a project and productive by Day 2 |
| **Reported result** | Estimated cost savings of **$650K+** across April 2025 – May 2026 |

### The problem this program solves

Contingent talent pools fail in a predictable way. An organization vets a group of
skilled designers, then holds them in a queue with no contact until a project appears.
By the time the project lands, the pool has gone cold: people have taken other work,
forgotten the house method, or never learned it in the first place. Ramp-up then
...
1. **Standby has to be worth something.** If participation is unpaid and voluntary, the
   program must return real value: craft upskilling, community, insider context, and a
   transparent path to being staffed.
...
|---|---|---|---|
| **1. Orientation** | Who we are, how projects run, what "done" means, code of conduct | Yes | One deck + acknowledgment form |
| **2. Craft Upskilling** | The house design method: learning design concepts, modular system, streamlined development, technical tracks | Yes | 5 modules + knowledge check |
| **3. Live Practice** | Recorded and live sessions: kickoff walkthroughs, quality labs, tool demos, showcases | No (recorded) | Rolling library + live invites |
| **4. Community & Continuity** | Newsletter, office hours, 1:1 coaching, discussion board, availability signalling | No | Recurring cadence |
...
| Day | Activity |
|---|---|
| Day 1 | Review orientation deck in full; begin/complete craft modules; prepare questions |
| Day 2 | Kickoff call with lead designer or manager (≤1 hr): resource walkthrough, scope, shadowing on first deliverable |
| Day 2–3 | Perform tasks outlined during orientation; focus on content and delivery improvements |
...
| Sub-lesson | Time |
|---|---|
| Learning design at the organization | 5 min |
| Learning objectives | 15 min |
| Scaffolding and problem-based learning | 15 min |

Covers: the definition of learning experience design; the house philosophy (market
insights + client collaboration + instructional design principles + agile methods,
oriented to human-centred design and real workforce problems); what a learning
...
> **SIMPLE** (plainly phrased, no double-barrelled objectives) · **CLEAR** (written for
> the "before" learner) · **VALUABLE** (worth a learner's or client's time and money) ·
> **REALISTIC** (reflects an action they'll actually take) · **MEASURABLE** (can be
> evaluated)

Each objective must be evaluable by at least one knowledge check (async) or a project,
exercise, or discussion (live). The module includes a weak → stronger → how-to-get-there
table and a graded worked example. It closes by separating **program goals** (why someone
buys the program; owned by product management; require learner action and often
...
1. **Start with why.** Adult learners are busy and want to know immediately how this
   applies to them.
2. **Show, don't tell.** The "I do, we do, you do" scaffold, with guidance decreasing
   as learners progress.
3. **Problem- and project-based learning.** Complex real-world problems instead of
   direct presentation of facts.

with weak/better practical examples per discipline, and the four-part **work-based
problem** structure: Scenario · Requirements · Issues · Practice.
...
| Block | Definition |
|---|---|
| **Microlesson** | The smallest unit of learning. 15–30 minutes, one skill, **one learning objective**, one activity. |
| **Module** | A self-contained package of related microlessons teaching a complete skill. |
| **Competency** | One or more modules representing a comprehensive skill set. |

Modules mix and match at the module level to compose new products for a client;
"modules of content should move together."
...
| Sub-lesson | Time |
|---|---|
| The modularization process | 5 min |
| Use cases | 10 min |
...
| # | Step | Estimate |
|---|---|---|
| 1 | Define the project (scope, goals, objectives, stakeholders, team) | ~4 h |
| 2 | Compile research | ~8–16 h |
| 3 | Conduct an audit of existing content and its objectives | ~8–16 h |
| 4 | Define competencies and modules | ~1–2 h |
| 5 | Draft the competency map | ~4 h |
| 6 | Write learning objectives | ~8–12 h |
| 7 | Develop the content | varies |
| 8 | Finalize the competency map | ~4–8 h |
| 9 | Update the central curriculum catalog | ~1 h |

**Four use cases** determine which steps you run:

| Use case | Steps |
|---|---|
| Net-new build | Skip step 3 (nothing to audit) |
| Partial new + pre-existing | All steps; audit pre-existing, define *missing* competencies and objectives |
...
LMS. Templates feed content into source control, slides, and the LMS. Access is
provisioned with a view licence.

For technical tracks, the module also covers designing coding courses in source control:
one organization holding a repository per reusable module (several hundred and
growing), a second organization holding one repository per course that acts as a table
of contents instructors fork per cohort, and learner access via LMS iframes rather than
direct repository links. Module templates exist for lessons,
labs, and projects.
...
| Sub-lesson | Time |
|---|---|
| Overview of streamlined development | 10 min |
| QA of learning design content | 5 min |

Streamlined development is defined by what it **is not**: not working faster, not working
more hours, not working more days. It **is**: building workflow systems with the right
tooling; templatizing and standardizing the learning experience; defining an MVP quality
level for intro products; protecting uninterrupted focus time; identifying and resolving
the problems that cause churn and rework; and operationalizing better role definition and
staffing models.

It sits inside the product development cycle at **Curriculum Development**, and only works
when Discovery and Planning have produced a locked scope and protected focus time.
...
the three assumptions those numbers depend on (no reusable modular content exists; the
...
Every live session is recorded and filed in a Recordings section, framed as optional
but valuable behind-the-scenes context on how the team makes decisions.
...
|---|---|---|
| **The Blast** (newsletter post) | Weekly–biweekly | Staffing wins, action items, ICYMI recaps, quality tip, invitations |
| **Open office hours** | Biweekly, 60–90 min | Drop-in; no agenda required; peer networking as much as leader access |
| **1:1 coaching** | On request | Project strategy, task walkthroughs, concerns |
| **Availability survey** | Quarterly | Capacity band, date range, industry experience tags |
| **Community discussion board** | Continuous | "What are you working on?" peer threads |
| **End-of-project survey** | Per engagement | Program and process feedback |
| **Scheduling poll** | Ad hoc | Choosing session times across a global, async pool |

**The Blast is the spine of the program.** A consistent structure repeated every issue:
...
## 5. Assessment and readiness gates

| Gate | Instrument | What it proves |
|---|---|---|
...
- Arrive at working sessions prepared and stay for the whole session
- Raise overruns promptly for review and resolution
- Represent the organization in client-facing settings: official channels and email only;
  no personal email addresses, no promotion of your own entities, services, or products
...
4. Deliverables provided in the established format

Paired with a "what good looks like" exemplar set: a good outline, a good storyboard,
a good final product, a good project or code sample.
...
The program deliberately uses a **free, familiar, low-friction course shell** rather than
a corporate LMS, because the audience is not yet contracted and cannot be given internal
system accounts.

| Layer | Function |
|---|---|
| Course shell (Google Classroom) | Single front door: announcements, modules, assessments, community board, meeting link |
| Announcement stream | The Blast; pinned welcome post with terms and to-dos |
| Classwork, organised by topic | Community Discussions · Orientation · Craft Upskilling · Recordings · Archived |
| Materials | PDF decks and videos attached per module with objective + timing in the description |
| Assignments | Forms for knowledge checks and acknowledgments |
| Attached meeting room | One persistent video link for office hours |
| Attached calendar | Session invites |
| Attached drive folder | Sample project kit and shared assets |
| Forms | Availability, acknowledgment, knowledge check, discovery, end-of-project survey |
...
Order the course shell by *what the participant needs next*, not by content type:

```
YOUR COMMUNITY DISCUSSIONS      ← belonging first
ORIENTATION                     ← required, ~1h
CRAFT UPSKILLING                ← required, ~130 min
RECORDINGS (CATCH UP!)          ← optional depth, always growing
[ARCHIVED]                      ← retired items kept visible, not deleted
```

Archiving rather than deleting is a small operational kindness: it preserves the
program's history and prevents "where did that go?" traffic.

### 7.3 Operating calendar

| Period | Operating focus |
|---|---|
| Quarter open | Availability form live; the ask repeated in every Blast until response rate is acceptable |
...
### 7.4 Roles to operate it

| Role | Responsibility |
|---|---|
| **Program owner** | Owns the shell, the Blast, office hours, the availability data, and matching |
| **Design manager** | Craft standards, technical module content, escalation |
| **Contributing practitioners** | Record walkthroughs and showcases from real projects |
...
A standby bench is only worth running if it costs less than it saves. Concentrating the
whole program in a single owner is what makes that arithmetic work.

**The containment.** Maintaining a pool of unassigned contractors is overhead charged
against no billable project. Spread across the learning management function, that
overhead taxes every manager's week on people who are not yet working. Concentrated in
one role, it is ring-fenced: the rest of the function keeps its attention on employees
and on actively contracted staff, and the standby population still gets a real program
instead of neglect. One person absorbing the pool is what buys everyone else their focus.
...
| Lever | Basis | Weight |
|---|---|---|
| Onboarding cost avoided | Orientation hours replaced, across the contractors who came through the program | A small fraction of the total |
| Rate differential | ~⅓ rate reduction applied to every delivered hour | Nearly all of it |
...
**On pool size.** The onboarding lever counts ~50 individual contractors moving through
the program in the period. That is people, not contracts: one contractor typically held
several assignments over time, so the wider contingent workforce was tracked in the
...
- One project channel per engagement; all project communication happens there
- Shared calendar and video for coordination; meeting cadence agreed at kickoff
- Deliverables tracked in a shared development planning sheet
- Planning documents in shared drive; technical content in source control
- Interim deadlines with clearly chunked work, agreed before the end of each working
  session and written into the working doc or agenda notes
...
### Content integrity
Curriculum is treated as brand and product line. Research-based, data-driven development
is required. Plagiarism is defined explicitly (taking someone else's work or idea and
misrepresenting it afterward) and named as grounds for removal from a project.
...
| Area | Guideline |
|---|---|
| **Ethical use & integrity** | Fact-check outputs; disclose AI-generated content; cite sources and avoid plagiarism |
| **Content quality** | Align to course objectives; edit for clarity and learner focus; hold tone and format consistent |
| **Inclusivity & accessibility** | Review for bias and fairness; avoid stereotypes; ensure readability and alt text |
| **Confidentiality & data privacy** | No sensitive learner data in LLMs; check before inputting internal documents; follow legal and organizational standards |
| **Human oversight** | Always review and edit; use LLMs for drafting, not finalizing; outputs that read as purely machine-generated are not accepted |
| **Continuous improvement** | Use design-lead feedback to improve LLM-assisted deliverables; keep current with the tooling |

The operating stance: AI for speed, **human in the loop** for inclusivity, relatability,
...
## 10. Honest assessment: what carried it, what needs planning for
...
- Themed months, which gave a standby population something to be *in* rather than
  waiting through
- A project kit that made day one on a real engagement recognizable before it happened
- Estimating the craft: the nine-step process with hour ranges, and the development
  week calculators with their assumptions stated
...
- **Content freshness.** Method decks carried version dates spanning multiple years while
  the surrounding tooling changed fast; at least one module shipped marked
  "work in progress."
...
## 11. Metrics worth instrumenting
...
| Is the program producing placements? | Placements per quarter; time from match to productive Day-2 kickoff |
| Is onboarding shortening ramp? | Days from contract to first accepted deliverable |
| Is quality holding? | Rubric scores at QA; volume of rework; QA hours per content hour |
...
## 12. Build-it-yourself checklist
...
- [ ] Break the house method into modules of 15–65 minutes; publish the objective and
      the runtime on every one
- [ ] Build two forms: an acknowledgment and a knowledge-check-plus-acknowledgment
- [ ] Build one quarterly availability form with capacity bands and experience tags
- [ ] Publish the staffing rule: no current availability record, no consideration
